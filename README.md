...
GitHub Release 当前只发布源码，不提供 Windows EXE。公开安装包需要代码签名和真实 Windows x64 安装、升级、备份恢复及卸载回归；不要从第三方渠道下载声称属于本项目的可执行文件。

应用不包含账号、云数据库、多人协作、多设备同步或自动更新。工作区、自动恢复点和手动 JSON 备份均为**本机明文数据**。请使用独立 Windows 账户、BitLocker 和受控的异地备份保护工资信息。

旧版备份仍可导入；从旧桌面安装迁移时，应先在旧应用导出 JSON，再在新版中手动恢复。
...
├── .github/             # CI、依赖更新和贡献模板
├── docs/                # 架构、数据安全与公开截图
├── electron/            # Electron 主进程、预加载与自动恢复点
├── shared/              # Web 与桌面共用的备份格式验证
├── src/                 # React 界面、工资逻辑和工作区操作
└── public/              # 项目自有静态资产
```
